...
namespace VirtualPhenix.Serialization
{
    /// <summary>
    ...
    /// 
    /// VP_ComplexFormatter.SaveJSONStringToBinaryFile(JsonString, PathToSave);
    /// VP_ComplexFormatter.SaveObjectToBinaryFile(JsonString, PathToSave);
    /// 
    /// * You can load data from binaryFile using 
    /// 
    /// VP_ComplexFormatter.LoadJSONStringFromBinaryFile(FilePath);
    /// 
    /// * And the same goes for DES Encryption:
    /// 
    /// VP_ComplexFormatter.SaveJSONStringToDESString(jsonString, path, keyToDecrypt);
    /// 
    ...
    /// </summary>
    public static class VP_ComplexFormatter
    {
        public static AESEncryption m_aesEncryption;
        ...
        public static string[] LoadAllLinesFromJSONFile(string path, Encoding encoding)
        {
            return  File.ReadAllLines(path, encoding);
        }

        public static string LoadJSONStringFromJSONFile(string path, Encoding encoding)
        {
        ...
        public static string LoadJSONStringFromObject<T>(T myobject)
        {
            return SerializeToJSON(myobject);
        }
        ...
        // OFFICIAL API https://docs.microsoft.com/es-es/dotnet/api/system.security.cryptography.aes?view=netframework-4.7.2
        ...
        public static void SaveObjectToAESEncryptedFile<T>(T myObjectToSave, string path)
        {
            string jsonString = SerializeToJSON(myObjectToSave);
            SaveStringToAESEncryptedFile(jsonString, path);
        }

        public static void SaveJSONToAESEncryptedFile<T>(string jsonPath, string path)
        {
            string jsonString = File.ReadAllText(jsonPath);
            SaveStringToAESEncryptedFile(jsonString, path);
        }

        public static void SaveJSONFileFromAESEncryptedFile<T>(string jsonPath, string path)
        {
            string jsonString = LoadJSONStringFromAESString(path);
            SaveJSONStringToJSONFile(jsonString, path);
        }

        public static void SaveStringToAESEncryptedFile(string jsonString, string path)
        {
            if (m_aesEncryption == null)
            {
                ...
                m_aesEncryption.myAes = myAes;
            }

            // Encrypt the string to an array of bytes.
            byte[] encrypted = EncryptStringToBytes_Aes(jsonString, m_aesEncryption.myAes.Key, m_aesEncryption.myAes.IV);
            string encryptedString = Encoding.ASCII.GetString(encrypted);
            File.WriteAllText(path, encryptedString);
        }

        public static string LoadJSONStringFromAESString(string path)
        {
            if (m_aesEncryption == null)
                return "";
            ...
            return DecryptStringFromBytes_Aes(bytes, m_aesEncryption.myAes.Key, m_aesEncryption.myAes.IV);
        }

        public static T LoadObjectFromAESEncryptedFile<T>(string path)
        {
            string jsonString = LoadJSONStringFromAESString(path);
        ...
        public static void SaveObjectToDESEncryptedBinaryFile<T>(T myObjectToSave, string path, string key = EncryptionSetup.PRIVATE_KEY)
        {
        ...
        public static void SaveJSONStringToDESEncryptedBinaryFile(string jsonString, string path, string key = EncryptionSetup.PRIVATE_KEY)
        {
            string encryptedString = SaveJSONStringToDESString(jsonString, path, key);
            SaveJSONStringToBinaryFile(encryptedString, path);
        }

        public static string LoadJSONStringFromDESEncryptedBinaryFile(string path, string key = EncryptionSetup.PRIVATE_KEY)
        {
            string encryptedString = LoadJSONStringFromBinaryFile(path);
            return LoadJSONStringFromDESString(encryptedString, key);
        }

        public static T LoadObjectFromDESEncryptedBinaryFile<T>(string path, string key = EncryptionSetup.PRIVATE_KEY)
        {
            string encryptedString = LoadJSONStringFromBinaryFile(path);
        ...
        public static void SaveJSONStringToDESFile(string jsonString, string path, string key = EncryptionSetup.PRIVATE_KEY)
        {
            try
            {
        ...
        public static string SaveJSONStringToDESString(string jsonString, string DESPath, string key = EncryptionSetup.PRIVATE_KEY)
        {
            try
            {
                Stream stream = File.Open(DESPath, FileMode.Create);
                BinaryFormatter bformatter = new BinaryFormatter();
                bformatter.Binder = new VersionDeserializationBinder();
                ...
                return File.ReadAllText(DESPath);
        ...
        public static void SaveBinaryFromDESFile(string DESPath, string BinaryPath, string key = EncryptionSetup.PRIVATE_KEY)
        {
            string jsonString = LoadJSONStringFromDESFile(DESPath, key);
    ...
    {
        public const string AES_PATH = "/AES/Data/Aes.dat";
