...
{
	public class AStarMovement : BehaviorDesigner.Runtime.Tasks.Movement.AstarPathfindingProject.IAstarAIMovement
	{
		...
		public override void OnStart()
		{
			...
			m_canRun = m_run.Value && m_runAnimation.Value != null && m_runAnimation.Value.AnimationName.IsNotNullNorEmpty();
			m_useAnimations = m_useAnimations.Value && m_walkAnimation.Value != null && m_walkAnimation.Value.AnimationName.IsNotNullNorEmpty();
		}
		...
		{
			base.OnDrawGizmos();
			if (m_debugTarget && m_started && agent != null && !agent.isStopped)
			{
				Gizmos.color = m_debugTargetPointColor;
				Gizmos.DrawSphere(agent.destination, m_debugTargetRadius);
			}
		...
		{
			if (agent.remainingDistance > m_distanceForChangingMovementAnimation.Value)
			{
				if (m_canRun)
				{
