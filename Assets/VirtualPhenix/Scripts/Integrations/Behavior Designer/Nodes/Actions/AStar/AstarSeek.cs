...
{
	[TaskDescription("Seek the target specified using AStar Pathfinding Project Pro.")]
	...
	public class AstarSeek : AStarMovement
	{
		[Header("Seek Properties"),Space]
		...
		[BehaviorDesigner.Runtime.Tasks.Tooltip("Variable used as toggle for stopping the process. Always returns Failure.")]
		public SharedBool m_allowGiveUp = true;
		...
		protected bool m_returnFailure = false;
		...
		public override void OnStart()
		{
			...
			m_canGiveUp = m_allowGiveUp.Value && m_maxDistanceToSeek.Value > 0;
	
			if (m_mainObject.Value == null)
				m_mainObject.Value = Owner.transform;
				
			SetDestination(Target());
		}
		
		public override void DoOnBlock()
		{
			base.DoOnBlock();
			
			if (m_stopSeeking.Value)
			{
				m_returnFailure = true;
			}
				
		}

		public override bool NeedToReturnFailure()
		{
			return m_returnFailure;
		}

		public override void UpdateBody()
		{
			m_distance = CheckDistanceToTarget();
			
			if (m_canGiveUp)
			{				
				if (m_stopSeeking.Value || m_distance >= m_maxDistanceToSeek.Value)
				{
					m_returnFailure = true;
					return;
				}
		...
		{
			m_isRunning = m_canRun && m_distance >= m_distanceToRun.Value;
			CheckIsRunning();
			SetDestination(Target());
		}
