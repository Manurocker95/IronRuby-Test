...
namespace VirtualPhenix.GridEngine
{
    ...
    public class VP_GridCharacterPathWalker : MonoBehaviour
    {
        ...
        public List<VP_GridTile> m_Path = new List<VP_GridTile>();
        ...
        public virtual void SetPath(List<VP_GridTile> path, bool andMove = false, Action onReachedDestination = null)
        {
            m_Path = path;
            m_NextPathIndex = 0;
        ...
        protected virtual void DetermineNextPathIndex()
        {
            ...
            if (m_Path[m_NextPathIndex] == _gridObject.CurrentGridTile)
            {
        ...
        /// <returns></returns>
        public virtual void DeterminePath(VP_GridTile targetGridTile, bool andMove = true, Action onReachedDestination = null)
        {
            m_NextPathIndex = -1;

            var newPath = VP_GridAStar.Search(_gridObject.CurrentGridTile, targetGridTile);
        ...
        public virtual void DeterminePath(Vector2Int targetGridPosition, bool andMove = true, Action onReachedDestination = null)
        {
            ...
            var newPath = VP_GridAStar.Search(_gridObject.CurrentGridTile, targetGridPosition);

            if (newPath != null && newPath.Count > 0)
            {
                var pathContainsTileAtPosition = newPath.Any(tile => tile.GridPosition == targetGridPosition);
