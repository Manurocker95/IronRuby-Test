...
namespace VirtualPhenix.Serialization
{
	...
	[Serializable]
	public abstract class SerializableDictionaryBase<TKey, TValue, TKeyStorage, TValueStorage> : SerializableDictionaryBase, IDictionary<TKey, TValue>, IDictionary, ISerializationCallbackReceiver, IDeserializationCallback, ISerializable
	{
		Dictionary<TKey, TValue> m_dict;
		[SerializeField]
		TKey[] m_keys;
		...
		protected abstract void SetValue(TValueStorage[] storage, int i, TValue value);
		protected abstract void SetValue(TKeyStorage[] storage, int i, TKey value);
		protected abstract TValue GetValue(TValueStorage[] storage, int i);
		...
		public void OnAfterDeserialize()
		{
			if (m_keys != null && m_keyStorage != null && m_values != null && m_keys.Length == m_values.Length)
			{
				m_dict.Clear();
				int n = m_keys.Length;
				for (int i = 0; i < n; ++i)
		...
		public void OnBeforeSerialize()
		{
			...
			m_keys = new TKey[n];
			m_values = new TValueStorage[n];
			m_keyStorage = new TKeyStorage[n];
			...
			foreach (var kvp in m_dict)
			{
				m_keys[i] = kvp.Key;
				SetValue(m_values, i, kvp.Value);
				SetValue(m_keyStorage, i, kvp.Key);
				++i;
			}
		}
	...
	public class SerializableDictionary<TKey, TValue, TKeyStorage, TValueStorage> : SerializableDictionaryBase<TKey, TValue, TKeyStorage, TValueStorage> where TValueStorage : SerializableDictionary.Storage<TValue>, new() where TKeyStorage : SerializableDictionary.Storage<TKey>, new()
	{
		public SerializableDictionary() { }
		public SerializableDictionary(IDictionary<TKey, TValue> dict) : base(dict) { }
		protected SerializableDictionary(SerializationInfo info, StreamingContext context) : base(info, context) { }
		...
		protected override void SetValue(TKeyStorage[] storage, int i, TKey value)
		{
			storage[i] = new TKeyStorage();
			storage[i].data = value;
		}
	}
