...
namespace XNodeEditor {
    [System.Serializable]
    public class NodeWindowContextMenuData
    {
        public GUIContent m_content;
        public bool m_on;
        public GenericMenu.MenuFunction m_func;

        public NodeWindowContextMenuData()
        {

        }

        public NodeWindowContextMenuData (GUIContent _cnt, bool _on, GenericMenu.MenuFunction _func)
        {
            m_content = _cnt;
            m_on = _on;
            m_func = _func;
    ...
    public partial class NodeEditorWindow : EditorWindow, IHasCustomMenu
    {
        ...
        public static List<NodeWindowContextMenuData> m_ctxMenuList = new List<NodeWindowContextMenuData>();
        ...
        {
            foreach (NodeWindowContextMenuData data in m_ctxMenuList)
                menu.AddItem(data.m_content, data.m_on, data.m_func);
        }

        /// <summary> Create editor window </summary>
        public static NodeEditorWindow Init() {
            NodeEditorWindow w = CreateInstance<NodeEditorWindow>();
            w.titleContent = new GUIContent(w.graph.windowName);
        ...
        public XNode.Node GetSelectedNode()
        {
            return (Selection.objects.Length > 1 ? Selection.objects[Selection.objects.Length-1] : Selection.objects[0]) as XNode.Node;
        }
        ...
        public static void Open(XNode.NodeGraph graph) {
            ...
            if (string.IsNullOrEmpty(graph.windowName))
                graph.windowName = "xNode";

            NodeEditorWindow w = GetWindow(typeof(NodeEditorWindow), false, graph.windowName, true) as NodeEditorWindow;
            w.wantsMouseMove = true;
            w.graph = graph;
        }
