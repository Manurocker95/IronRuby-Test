...
namespace XNodeEditor {
    public static class NodeEditorPreferences {
        ...
        public class Settings : ISerializationCallbackReceiver {
            ...
            [SerializeField] public Color32 m_nodePortColor = new Color(0.18f, 0.18f, 0.18f, 1f);
        ...
        public static SettingsProvider CreateSettingsProvider()
        {
            if (lastEditor != null)
            {
                SettingsProvider provider = new SettingsProvider("Preferences/" + lastEditor.GetPreferencesName(), SettingsScope.User)
                {
                    guiHandler = (searchContext) => { XNodeEditor.NodeEditorPreferences.PreferencesGUI(); },
                    keywords = new HashSet<string>(new[] { "xNode", "Dialogue", "node", "editor", "graph", "connections", "noodles", "ports" })
                };
                return provider;
            }
        ...
        private static void TypeColorsGUI(string key, Settings settings) {
            ...
            /*
            Color nodePortColor = settings.m_nodePortColor;
            EditorGUI.BeginChangeCheck();
            EditorGUILayout.BeginHorizontal();
            nodePortColor = EditorGUILayout.ColorField("Node Ports:" , nodePortColor);
            EditorGUILayout.EndHorizontal();
            if (EditorGUI.EndChangeCheck())
            {
                settings.m_nodePortColor = nodePortColor;
                SavePrefs(key, settings);
                NodeEditorWindow.RepaintAll();
            }
            */
        ...
        public static Color GetNodePortColor()
        {
            return GetTypeColor(typeof(XNode.NodePort));//settings[lastKey].m_nodePortColor;
        }
